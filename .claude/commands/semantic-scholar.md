...
- **API key:** `~/.secrets/s2_api_key` (chmod 600, on NFS home — not Dropbox). Consume it **inline** and never echo it: `S2_API_KEY="$(cat ~/.secrets/s2_api_key)" python3 tools/s2lit.py …`. Do not print, cat, or paste the key; do not put it in a committed file or in shell history.
...
1. If `./tools/s2lit.py` does **not** exist, create it verbatim from the embedded code block at the bottom of this file (`mkdir -p tools` first). If it already exists, reuse it — do not overwrite.
2. `mkdir -p papers`.
3. Detect the key once: `test -s ~/.secrets/s2_api_key && echo keyed || echo anonymous`. If anonymous, note that Phase 1 will use the WebSearch fallback and skip the `s2lit.py search`/`expand` calls (they would just 429).
4. If `papers/manifest.json` already exists, read it — this is an additive re-run; everything dedupes against it.
...
- **If keyed and depth warrants:** also run `S2_API_KEY="$(cat ~/.secrets/s2_api_key)" python3 tools/s2lit.py search "<q1>" "<q2>" … --out s2results.json` for citation-ranked hits.
...
4. Download with `S2_API_KEY="$(cat ~/.secrets/s2_api_key)" python3 tools/s2lit.py fetch <id:theme> … --dir papers --manifest papers/manifest.json`, or `--from <file>` for a curated list (lines `id[,theme[,note]]` or a JSON array). The key isn't required for arXiv PDF downloads, but passing it inline is harmless and keeps one pattern.

For `expand` mode: `python3 tools/s2lit.py expand <ids> --out expand_candidates.json` (or no ids → seeds from the manifest), review the seed-degree-ranked candidates, curate, then fetch as above.
...
| mode | phases | output |
|-|-|-|
| `quick` | 1 | ranked candidate list, no files beyond `tools/s2lit.py` + `s2results.json` |
...
## Embedded tool — write to `./tools/s2lit.py` if absent
