...
4. **Commit** one simplification per commit, and never amend — the target commit is left exactly as it is, whether or not it is `HEAD` and whether or not it is pushed.
   - For a commit or series target, commit `fixup! <subject of the commit being fixed>` with a one-line body naming what the iteration did, for the user to autosquash when they choose (interactive rebase is unavailable here). Pick the commit whose lines the iteration touched; when a series makes that ambiguous, fix up the newest one it touches.
   - For a file or directory target, there is nothing to fix up: commit normally, e.g. "Use early return to reduce nesting in process()".
   - Report the autosquash command the user will need (`git rebase -i --autosquash <base>`), and never run it or otherwise rewrite history yourself.
