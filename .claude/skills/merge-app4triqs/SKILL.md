...
If Step 2 flagged a migration — the skeleton swapping a whole tool or framework rather than
evolving a file — stop and read `${CLAUDE_SKILL_DIR}/references/tool-migrations.md`
before touching it. Line-by-line merging is the wrong model there, and the two failure modes
(silent loss of an app customization in a superseded file, silent leakage of a C++-only
toolchain into a pure-Python app) are invisible in `git status`. Handle one migration at a
time, with the user in the loop. If Step 2 flagged none, skip to Step 5.
